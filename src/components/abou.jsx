...
const zertifikate = {
  ...
  JavaScript: [
    { name: "Learn JavaScript Course", url: "/Zertifikate/Learn JavaScript Course.pdf" }
  ],
  "Navigation Design": [
    { name: "Learn Navigation Design Course", url: "/Zertifikate/Learn Navigation Design Course.pdf" }
  ],
  "Color Design": [
    { name: "Learn Color Design Course", url: "/Zertifikate/Learn Color Design Course.pdf" }
  ],
  "Generative AI": [
    { name: "Intro to Generative AI Course", url: "/Zertifikate/Intro to Generative AI Course.pdf" }
  ],
  "Web Deployment": [
    { name: "How to Deploy a Website Course", url: "/Zertifikate/How to Deploy a Website Course.pdf" }
  ]
};
    ...
    <section className="py-20 px-6 md:px-12 lg:px-20 bg-background">
      <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center gap-12">
        ...
        <div className="flex-1">
          ...
          <p className="text-lg text-textColor mb-6">
            Hi, ich bin Hannes, ein kreativer und leidenschaftlicher
            Frontend-Entwickler aus Deutschland. Ich liebe es, moderne, performante
            und ästhetische Webanwendungen zu bauen, die nicht nur gut aussehen,
            sondern sich auch richtig gut anfühlen. Mein Fokus liegt auf React,
            Astro, Tailwind CSS und allem, was das Web schöner macht!
          </p>
          <div className="mb-6">
            <h3 className="text-2xl font-semibold text-headingColor mb-2">Skills</h3>
            <div className="flex flex-wrap gap-3">
              {Object.keys(zertifikate).map((skill) => (
                <button
                  key={skill}
                  className="bg-secondary/20 text-headingColor px-3 py-1 rounded-full text-sm font-medium cursor-pointer hover:bg-secondary/40 transition"
          ...
          >
            <span className="font-semibold text-headingColor">Fun Fact:</span> Ich liebe
            es, neue Technologien auszuprobieren und bin immer auf der Suche nach kreativen
            Lösungen!
          </div>
        </div>
      </div>
      ...
      {openSkill && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-white dark:bg-background p-8 rounded-lg shadow-lg max-w-md w-full relative">
            <button className="absolute top-2 right-2 text-xl" onClick={() => setOpenSkill(null)}>×</button>
            <h3 className="text-2xl font-bold mb-4 text-headingColor">{openSkill} Zertifikate</h3>
            <ul className="list-disc pl-5 text-textColor">
              {zertifikate[openSkill].map((zert, i) => (
                <li key={i} className="mb-2">
                  {zert.type === "image" ? (
                    <div>
                      <span className="block mb-1">{zert.name}</span>
                      <img src={zert.url} alt={zert.name} className="max-w-xs max-h-48 rounded shadow" />
                    </div>
                  ) : zert.url ? (
                    <a
                      href={zert.url}
